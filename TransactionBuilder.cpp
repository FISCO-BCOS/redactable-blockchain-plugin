...
crypto::HashType TransactionBuilder::calculateTransactionDataHash(
    CryptoType _cryptoType, const bcostars::TransactionData& _transactionData, const std::string& _extraData)
{
    ...
    return _transactionData.hash(cryptoSuite->hashImpl(), _extraData);
}
    ...
    const std::string& _abi, int64_t _blockLimit, int32_t _attribute, const std::string& _extraData)
{
    ...
    auto transactionDataHash =
        calculateTransactionDataHash(_keyPair.keyPairType(), *transactionData, _extraData);
